...
def horizontal_flip(prob, images, boxes=None):
    ...
    if np.random.uniform() < prob:
        ...
        # [2, 3, 64, 224, 224]
        # raise RuntimeError(images.shape)
...
def _get_param_spatial_crop(
    scale, ratio, depth, height, width, num_repeat=10, log_scale=True, switch_hw=False
):
    """
    ...
    for _ in range(num_repeat):
        ...
        if log_scale:
            ...
            aspect_ratio_1 = math.exp(random.uniform(*log_ratio))
            aspect_ratio_2 = math.exp(random.uniform(*log_ratio))
            # raise RuntimeError(ratio, aspect_ratio_1, aspect_ratio_2)
        else:
            aspect_ratio = random.uniform(*ratio)

        d = int(round(depth * aspect_ratio_1))
        w = int(round(math.sqrt(target_area * aspect_ratio_2)))
        h = int(round(math.sqrt(target_area / aspect_ratio_2)))

        if np.random.uniform() < 0.5 and switch_hw:
            w, h = h, w

        # raise RuntimeError(aspect_ratio_1, d, depth, w, width, h, height)
        if 0 < d <= depth and 0 < w <= width and 0 < h <= height:
            k = random.randint(0, depth - d)
    ...
    # raise RuntimeError(in_ratio, ratio)
...
def random_resized_crop(
    ...
    k, i, j, d, h, w = _get_param_spatial_crop(scale, ratio, depth, height, width)
    # [i, j, h, w] = [0, 213, 64, 86]
    # scale, ratio = [0.3, 1.0], [0.75, 1.35]
    # images = [2, 3, 64, 512, 512]
    # raise RuntimeError(k, i, j, d, h, w, ratio, images.shape)

    # raise RuntimeError(images.shape)
    # [2, 3, 64, 512, 512]
    cropped = images[:, :, k:k + d, i:i + h, j:j + w]
    # raise RuntimeError(target_depth, target_height, target_width)
