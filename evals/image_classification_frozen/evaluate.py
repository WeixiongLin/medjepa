...
def calculate_mAP(labels, logits):
    ...
    for class_i in range(n_classes):
        ...
        # raise RuntimeError(type(labels), type(scores), class_i)
        # labels, scores = labels.cpu().detach().numpy(), scores.cpu().numpy()

        # raise RuntimeError( labels[:, class_i] )
        # labels, scores = [151, 5569], [151, 5569]
        # raise RuntimeError( labels[:, class_i], scores[:, class_i] )
        precision, recall, _ = precision_recall_curve(labels[:, class_i], scores[:, class_i])
        # raise RuntimeError( precision.shape, recall.shape )  # [133], [133]
